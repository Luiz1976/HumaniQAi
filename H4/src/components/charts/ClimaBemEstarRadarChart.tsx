...
interface ClimaBemEstarRadarChartProps {
...
// Cores modernas e profissionais para o radar - Tema Verde/Teal
const RADAR_COLORS = {
  fill: 'url(#radarGradient)', // Gradiente moderno
  fillOpacity: 0.3,
  stroke: '#14b8a6', // teal-500 vibrante
  strokeWidth: 3,
  dot: '#0d9488', // teal-600 
  dotActive: '#0f766e', // teal-700 para hover
  grid: '#e2e8f0', // slate-200
  gridSecondary: '#f8fafc', // slate-50
  accent: '#10b981', // emerald-500 para destaques
};

export function ClimaBemEstarRadarChart({ resultado }: ClimaBemEstarRadarChartProps) {
  // Validação defensiva
      ...
      <Card className="bg-white shadow-sm border border-slate-200/60">
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-xl">
            ...
            Perfil Organizacional - Visão Radar
          </CardTitle>
        </CardHeader>
        <CardContent>
  ...
  const data: DimensaoData[] = Object.entries(resultado.dimensoes)
    ...
    .map(([dimensaoId, dados]) => ({
      ...
      percentual: typeof dados.percentual === 'number' 
        ? dados.percentual 
        : Math.round(((dados.media ?? 0) / 5) * 100),
        ...
        <div className="mt-8 space-y-4">
          <div className="text-center">
            <p className="text-base font-semibold text-slate-800 mb-1">
              Perfil Organizacional Detalhado
            </p>
            <p className="text-sm text-slate-600">
              Visualização das quatro dimensões do clima organizacional e bem-estar psicológico
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 gap-3 text-sm">
            {data.map((item, index) => (
              <div 
                key={index} 
                className="group flex items-center justify-between p-4 bg-gradient-to-r from-teal-50 to-white rounded-xl border border-teal-200/80 shadow-sm hover:shadow-md transition-all duration-300 hover:scale-[1.01]"
              >
          ...
          <div className="flex flex-wrap items-center justify-center mt-6 p-4 bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg border border-teal-200/50">
            <div className="flex flex-wrap items-center justify-center gap-6 text-xs text-slate-600">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="whitespace-nowrap">25% - Moderado</span>
              </div>
              <div className="w-px h-4 bg-slate-300 hidden sm:block"></div>
              <div className="flex items-center gap-2">
                <div className="w-3 h-3 rounded-full bg-teal-500"></div>
                <span className="whitespace-nowrap">50% - Bom</span>
              </div>
              <div className="w-px h-4 bg-slate-300 hidden sm:block"></div>
              <div className="flex items-center gap-2">
                <div className="w-3 h-3 rounded-full bg-emerald-600"></div>
                <span className="whitespace-nowrap">75% - Excelente</span>
              </div>
              <div className="w-px h-4 bg-slate-300 hidden sm:block"></div>
              <div className="flex items-center gap-2">
                <div className="w-3 h-3 rounded-full bg-emerald-800"></div>
                <span className="whitespace-nowrap">100% - Ideal</span>
              </div>
            </div>
          </div>
        </div>
...
export default ClimaBemEstarRadarChart;